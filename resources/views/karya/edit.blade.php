        ...
        <form action="{{ route('karya.update', $karya->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select name="jenis"
                    ...
                    <option value="buku" {{ old('jenis', $karya->jenis) == 'buku' ? 'selected' : '' }}>Buku</option>
                    <option value="novel" {{ old('jenis', $karya->jenis) == 'novel' ? 'selected' : '' }}>Novel</option>
                    <option value="cerpen" {{ old('jenis', $karya->jenis) == 'cerpen' ? 'selected' : '' }}>Cerita Pendek</option>
                    <option value="artikel" {{ old('jenis', $karya->jenis) == 'artikel' ? 'selected' : '' }}>Artikel</option>
                </select>
            </div>
            ...
            <div class="mb-2">
                ...
                <textarea id="editor" name="isi" rows="10"
                    class="w-full p-3 border rounded-lg focus:ring focus:ring-blue-200"
                    placeholder="Mulai tulis karya Anda..." required>{{ old('isi', $karya->konten) }}</textarea>
            </div>
