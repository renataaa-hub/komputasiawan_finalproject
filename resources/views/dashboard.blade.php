        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 bg-white rounded-lg shadow-sm p-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    @if(!$search)
                    <a href="{{ route('karya.index') }}" class="text-blue-600 hover:text-blue-700 text-sm font-medium">
                        Lihat Semua →
                    </a>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($karya as $item)
                        @php
                            $statusColor = $item->status == 'draft' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800';
                            $isMyKarya = $item->user_id == Auth::id();
                        @endphp
                        <div class="bg-gray-50 rounded-xl border border-gray-200 p-5 hover:shadow-lg transition transform hover:-translate-y-1">
                            
                            @if($item->cover)
                            <img src="{{ asset('storage/' . $item->cover) }}" alt="{{ $item->judul }}" class="w-full h-40 object-cover rounded-lg mb-3">
                            @endif

                            <h3 class="text-xl font-semibold mb-1 text-gray-800">{{ $item->judul }}</h3>
                            
                            <div class="flex items-center gap-2 mb-2">
                                <p class="text-sm text-gray-500 capitalize">{{ $item->jenis ?? 'Tidak ada jenis' }}</p>
                                @if($search)
                                <span class="text-xs text-gray-400">•</span>
                                <p class="text-sm text-gray-500">oleh {{ $isMyKarya ? 'Saya' : $item->user->name }}</p>
                                @endif
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <span class="inline-block text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded mr-1 mb-1">
                                    {{ trim($tag) }}
                                </span>
                                @endforeach
                            </div>
                ...
                <div class="text-center py-12">
                    ...
                    @else
                    <a href="{{ route('karya.create') }}" class="text-blue-600 hover:underline">Buat karya pertama Anda</a>
                    @endif
                </div>
                @endif
            </div>
